...
export const featuredProjects = [
  {
    name: 'Felix UI',
    intro:
      'Felix UI is an open-source react component library that extends as a CSS styling framework, which helps developers to use the best styles and components, by using pre-defined class names. Felix UI helps you to quickly build your dream project, by focusing much on functionality, because we take care of your designs.',
    features: [
  ...
  },
  {
    name: 'Felix Social',
    intro:
      "A social media platform inspired from instagram and twitter and mixing it to one to provide users the best of both platform. 'Share moments' - 'Connect' - 'Know the world'",
    features: [
  ...
  },
  {
    name: 'Felix Store',
    intro:
      'An organic groceries portal for every health-conscious people out there. This shop is the one-stop solution for any organic need, be it fruits or plants for your beautiful garden, Felix Store has it all.',
    features: [
      'Complete checkout process',
      'Product filter for easy access',
      'Engaging design',
      'Profile dashboard for order history',
    ],
  ...
  },
  {
    name: 'Felix TV',
    intro:
      'A digital content platform around farming, research documentaries, and gardening for those who are trying to figure out how to save our green planet from global warming. Join Us!',
    features: [
      'Playlist management',
      'Integrated notes with timestamp',
      'Great user experience',
    ],
...
export function loadStars(count) {
  ...
  let blank =
    split.length == 2 ? 4 - parseInt(split[0]) : 5 - parseInt(split[0]);
  let arr = [];
  let i = 0;
  while (i < split[0]) {
...
export function scroll(el, screen) {
  if (el) {
    ...
    lastScrollPosition = position =
      lastScrollPosition > position && screen ? position - 60 : position;
    gsap.to(window, {
      duration: calculateDuration(position),
      scrollTo: position,
      ease: 'power2',
    });
  } else {
    gsap.to(window, {
      duration: calculateDuration(0),
      scrollTo: 0,
      ease: 'power2',
    });
  }
